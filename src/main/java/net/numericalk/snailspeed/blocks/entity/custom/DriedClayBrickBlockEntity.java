...
public class DriedClayBrickBlockEntity extends BlockEntity {
    ...
    private void fireClayBrick(World world, BlockPos pos, BlockState state) {
        if (world.isClient()) {
            return;
        }
        world.setBlockState(pos, SnailBlocks.FIRED_BRICK.getStateWithProperties(state));
    }
    ...
    private boolean isDriedClayBrick(BlockState state) {
        return state.isOf(SnailBlocks.DRIED_CLAY_BRICK);
    }
}
